class Population {
	...
	//------------------------------------------------------------------------------------------------------------------------------------------
	//mutates all the brains of the children
	mutateOffspring() {
		for (var i = 1; i < this.players.length; i++) {
			this.players[i].brain.mutate(this.innovationHistory, this.players[i].deathByDot, this.players[i].deathAtStep);
	...
	setBestPlayer() {
		...
		if (max > this.bestFitness) {
			...
			this.genPlayers.push(this.players[this.bestPlayer].getChild());
		}
